-- @弃用
-- 查询在剩余交易期耗尽后，已经完成交易的转让记录，并将这些记录的状态更改为5（转让完成）
-- 参数：
-- 		rightnow	当前时间
...
(
	...
	(
		...
		where 
			t.bookfk = b.id and b.recordz = rz.id and b.booktype='Z' and t.tradetype='Z' and t.status in (3,6,7,8) and rz.endlimit <= :rightnow
		group by rz.id
	) as table2
