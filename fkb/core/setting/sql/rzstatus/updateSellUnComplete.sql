-- @弃用
-- 查询在剩余交易期耗尽后，尚未完成所有交易的转让记录，并将这些记录的状态更改为6（系统结束）
-- 参数：
-- 		rightnow	当前时间

update fm_sell set status=6 where id in
...
(
	...
	(
		...
		where 
			t.bookfk = b.id and b.recordz = rz.id and b.booktype='Z' and t.tradetype='Z' and rz.endlimit <= :rightnow
		group by rz.id
	) as table2
