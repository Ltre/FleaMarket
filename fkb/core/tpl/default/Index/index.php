...
?>
    <h4 style="cursor: pointer;" opt="sell" fts="<?php if($fleatypes)$fts=null;foreach ($fleatypes as $fleatype){$fts.=$fleatype->id.'|';}print trim($fts,'|');?>" maxftlen="<?php print count($fleatypes)?>" shell="<?php print "fleatypeSearch|sell|{$fleatypes[rand(0, count($fleatypes)-1)]->id}|10|0"?>" onclick="javascript:click_underft(this);">随便搜搜(*^__^*).</h4>
	<!-- <h4>Theme Color Selector</h4> -->
    </div>
<!-- BEGIN HEADER -->
<header class="clearfix">
<!-- BEGIN LOGO -->
<a id="headerlink" href="../" title="华软旧货市场"><img id="logo" src="res/Matrix/images/logo.png" alt="logo"/><span id="sitename"><font face="黑体">华软旧货市场</font></span></a>
<!-- END LOGO -->
...
if($tile1['result'])
{
	$tile1_rzimg = FleaoptUtil::getFleaImgUrl($tile1['result']['rzid'], 'sell');//旧货信息封面
...
<a href="#article-1" class="lightbox" rel="section">
    <div class="tile large live" data-stops="0,25%,50%,75%,100%" data-speed="3000" data-delay="0" data-direction="horizontal" data-stack="true">
        ...
        <span class="tile-cat red">最新的转让：<?php print $tile1['result']['title']?></span>
    </div>
</a>
...
$tile3_rzimg = FleaoptUtil::getFleaImgUrl($tile3_rs ? $tile3['result']['rzid'] : 0, 'sell');//旧货信息封面
...
$tile4_rzimg = FleaoptUtil::getFleaImgUrl($tile4_rs ? $tile4['result']['rzid'] : 0, 'sell');//旧货信息封面
...
$tile5_rzimg = FleaoptUtil::getFleaImgUrl($tile5_rs ? $tile5['result']['rzid'] : 0, 'sell');//旧货信息封面
...
<a href="#quotation-1" class="lightbox" rel="section">
    ...
    <div class="lb-quote">
    在前两行除了第一个大磁贴和我，其它的都是按随机的旧货分类来展示的内容。另外，第二行右侧边缘有个传送门<font color="red"><b>＂＞＂</b></font>，点击可以查看求购信息
...
$tile7_rzimg = FleaoptUtil::getFleaImgUrl($tile7_rs ? $tile7['result']['rzid'] : 0, 'sell');//旧货信息封面
...
$tile8_rzimg = FleaoptUtil::getFleaImgUrl($tile8_rs ? $tile8['result']['rzid'] : 0, 'sell');//旧货信息封面
...
?>
    <div class="tile large live exclude" data-stack="true" data-stops="0,18%" data-speed="699" data-delay="500">
        ...
        <div class="live-back themecolor">
        	<span class="tile-text tweeter">最近发布的转让</span>
        </div>
    </div>
...
<a href="#article-3" class="lightbox" rel="section">
    <div class="tile medium" style="background-color: silver;">
        ...
        <span class="tile-date blacktxt"><span class="date">最多</span><span class="month">货源</span></span>
    ...
    <br>
    <div class="article-date tealtxt"><span class="date">最多</span><span class="month">的货源</span></div>
    <h1 class="lb-title"><?php print '［分类］'.$tile13['fleatype']?></h1>
    <div class="lb-excerpt">
        <p>现在查看的是货源最多的分类。</p>
        <p class="exp-button"><a href="javascript:;" style="font-size: 28px;" onclick="javascript:click_tile13(this);" shell="<?php print $tile13['shell']?>">点击查看该分类下的转让记录</a></p>
    </div>
...
<script type="text/javascript" src="res/js/index.js"></script><!-- res/Matrix/scripts/matrix.js -->
