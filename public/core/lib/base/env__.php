 ...
 * URL SHELL 的含义：见ActionUtil::numOfShellArgs()  @  /public/core/lib/action__.php
 ...
 * 其声明方式：class Xxxx {}
 */
define('ENTITY_DIR', 'public/core/entity/');
 ...
 * 如果想要通过URL[http://server.com/ItemName/?xxx=shell|*]的形式调用其中的方法，则需要配置ActionUtil::numOfShellArgs() @ /public/core/lib/base/action__.php，详见注释。
 ...
 */
define('ACTION_DIR', 'public/core/action/');
 ...
 */
define('FILTER_DIR', 'public/core/filter/');

/*
 * 用户扩展库文件默认保存位置是'public/core/lib/ext'。
 * 格式：路径要以项目根目录为起点，不能以“/”开头，必须以“/”结尾。
 * 如果要配置多个保存位置，就需要用“|”隔开。如：'public/core/lib/ext/|core/lib/abc/|abc/def/'。
 */
define('USER_LIB_DIRS', 'public/core/lib/ext/');

/*
 * php的const、define、URL指令等等的文件位置，这些文件名格式为：*.php。
 * 默认值为：'public/core/config/'。
 * 格式：路径要以项目根目录为起点，不能以“/”开头，必须以“/”结尾
 * 如果要配置多个保存位置，就需要用“|”隔开。如：'public/core/config/|core/config/myconfig/'。
 */
define('USER_CONFIG_DIRS', 'public/core/config/');

/*
 * XxxxAction默认模板目录，用于与输出Action方法同名的模板文件。
 * 默认保存于/public/core/tpl/default/。
 * 模板文件保存形式：先在public/core/tpl/default/新建名为Action名称的文件夹，再在这个文件夹中新建与Action方法同名的模板文件，后缀名限定为.php。
 * 输出方式：必须由Action的tpl()方法输出。链接将无法直接访问这类文件。
 * 参数支持：由XxxxAction的基类ActionUtil::getTplArgs() 提供带索引数组形式的参数，以供模板文件内脚本使用。
 */
define('ACTION_TEMPLATE_DIR', 'public/core/tpl/default/');


/*
 * 普通模板目录路径（其它模板目录路径），是可输出页面所在的路径，用于存放.php为后缀名的模板文件。
 * 通过包含这里的页面，就可以实现页面输出，这么做可以隐藏页面的真实路径。
 * 默认路径是public/core/tpl/ 。
 * 赋值格式：要以项目根目录为起点，不能以“/”开头，必须以“/”结尾。
 * 链接访问方法：http://server.com/ItemName/?xxx=模板文件参数
 * 可以要再细分目录存放模板文件，如果想通过链接访问到再深一层的文件，则需要用“-”代替“/”。
 * 如：http://server.com/ItemName/?xxx=a-b-c 将默认访问到public/core/tpl/a/b/c.php。
 * 因此模板文件名的命名规范是：1、不含“-”；2、必须以.php为后缀名。
 */
define('OTHER_TEMPLATE_DIR', 'public/core/tpl/other/');


/*
 * 配置403页面的路径
 * 默认值：public/core/tpl/403.php
 */
define('PAGE_403', 'public/core/tpl/403.php');
 ...
 * 以下是示例常量
 */
define('JQUERY_LIB_PATH', 'public/res/js/jquery-1.8.3.min.js');	//jQuery.js路径
