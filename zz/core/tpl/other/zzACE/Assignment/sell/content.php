...
<div class="row">
	<div class="col-xs-12">
<?php 
$from = intval(($self['pageOffset']-1)*$self['num']+1);//原为$from = intval(($self['pageOffset']-1)*$self['num']+1)
$to = intval($from+$self['num']-1);
$to = $to > $self['total'] ? $self['total'] : $to;
$from = !$to ? 0 : $from;
?>
		<h3 class="header smaller lighter blue">当前显示第 <span id="rs_from"><?php print($from) ?></span> 到 <span id="rs_to"><?php print($to) ?></span> 条记录，共<span id="rs_total"><?php print($self['total']) ?></span>条<font color="red">转让</font>记录</h3>
...
<?php foreach ($self['records'] as $record ){ ?>
					<tr>
						...
						<td shell="getFleaDetails" params="sell|<?php echo $record['titlecode'] ?>|self|<?php print($self['num'].'|'.$from)?>" style="cursor: pointer;">
...
case 4:echo '<span class="label label-sm label-success">物主关闭</span>';break;
case 5:echo '<span class="label label-sm label-success">转让完成</span>';break;
...
?>						
						...
						<td>
							<div class="visible-md visible-lg hidden-sm hidden-xs action-buttons">
								<a shell="getFleaDetails" params="sell|<?php echo $record['titlecode'] ?>|self|<?php print($self['num'].'|'.$from)?>" style="cursor: pointer;" class="blue">
									<i class="icon-zoom-in bigger-130"></i>
								</a>
								<a shell="pageToCreateFleaInfo" params="editsell|<?php echo $record['titlecode'] ?>" style="cursor: pointer;" class="hide green ai-skip">
									<i class="icon-pencil bigger-130"></i>
								</a>
								<a id="delFleaInfo" shell="delFleaInfo" params="sell|<?php echo $record['titlecode']?>" style="cursor: pointer;" class="red ai-ajax">
...
for ($i = 1 ; $i <= $pageTotal ; $i ++){
	...
	print('<button class="btn '.$pageOffset.' ai-skip" shell="listSelfSell" params="'.$num.'|'.strval(($i-1)*$num).'">'.$i.'</button>');
}
